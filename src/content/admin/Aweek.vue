...
import { do_searchbook, do_searchtime, do_renewal, do_return, borrowbook } from '../../utils/operationBook.js';
export default {
  components: {
    vRecordtitle
  },
    ...
    getData() {
      this.$ajax.post('/admin/sevenDaysBorrow').then((res) => {
        ...
        for (const book of res.data.data) {
           ...
           * phone：借阅人手机号
           */
          let { title, borrowTime, borrowUser, borrowCycle, isLending, returnTime, _id, username, phone } = book
          this.tableData.push({
